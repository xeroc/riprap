...
const LAUNCH_REASONS = [
  ...
  {
    head: "measured in weeks",
    body: "every pool closes on a known clock and publishes its numbers — join rate, claims, verdicts, returns.",
  },
];

const Pooled: FC<{ children: string }> = ({ children }) => (
  <span data-num className="text-accent [font:var(--riprap-mono-number)]">
    {children}
  </span>
);
      ...
      <div className="flex w-full items-start gap-10">
        <div className="flex flex-1 flex-col gap-6">
          {LAUNCH_REASONS.map((r, i) => (
            <div key={r.head} className="flex flex-col gap-1" style={rise(frame, 24 + i * 16)}>
        ...
        <div
          className="flex w-[44ch] flex-col gap-4 border border-hairline bg-(--riprap-surface-card) p-8"
          style={rise(frame, 48)}
        >
          ...
          <div className="text-muted [font:var(--riprap-body-sm)]">
            Olympia, London · 15–17 November 2026 · 8,000+ attendees
          </div>
          <div className="flex flex-col gap-2 border-t border-hairline pt-4 text-muted [font:var(--riprap-body-sm)]">
            <div>
              tiers <Pooled>$10/$20/$40</Pooled> — payouts capped at{" "}
              <Pooled>$1,000/$2,000/$4,000</Pooled>
            </div>
            <div>
              worked example: <Pooled>1,000 × $20</Pooled> pooled → 4 approved claims ×{" "}
              <Pooled>$2,000</Pooled> paid → <Pooled>$12</Pooled> back each
            </div>
            <div>quiet event: every cent returns, then the pool dissolves</div>
          </div>
        </div>
      </div>
    </SlideFrame>
...
export const SLIDES: SlideDef[] = [
  ...
  },
  {
    ...
    label: "the launch",
    notes:
      "35s. Events are the door, chosen for three reasons and none of them market size: the cleanest legal surface (discretionary mutual, crypto-native members, bounded window — no counsel spend at pilot scale); the organizer is a B2B2C channel — one sale brings the attendee list; and every pool closes in weeks and publishes its numbers. First pool: Blade Pool at Breakpoint, Olympia London, 15–17 Nov 2026, 8,000+ attendees; tiers $10/$20/$40 capped at $1k/$2k/$4k; worked example: 1,000 × $20 → 4 claims × $2,000 → $12 back each; quiet event: every cent returns. Then the 2027 circuit — the PMF signal and the primary milestone: organizer-initiated pools, an organizer who shows up without us.",
