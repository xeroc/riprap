...
|SDK facades|`packages/{pool,hanse}/src/` (`index.ts`, `pdas.ts`, `fetch.ts`, pool: `token.ts` + colocated tests)|Hand-written public surface (PDA derivation, account fetchers; ATA derivation + SPL program ids in `packages/pool/src/token.ts` via `@solana-program/token`) re-exported together with the generated client; `@riprap/hanse` depends on `@riprap/pool` and re-exports its token surface.|
...
|Frontends|`apps/landing/` (single-entry Vite + React, hash-routed: `/` platform landing, `#/2026-breakpoint-blade-pool` pool page, `#/app` member wallet surface, `#/app/file-claim` payout-request wizard — router in `src/main.tsx`, lazy route modules `src/{pool,app}/entry.tsx` + `src/app/file-claim/entry.tsx`, old MPA paths redirect via stubs in `public/{2026-breakpoint-blade-pool,app}/index.html`), `apps/pitch-seed-raise/`|React + Vite (one `index.html` entry); consume `@riprap/ui`. The head (title/OG/canonical/JSON-LD) is static in `index.html` (platform); route `<title>`s swap in the router — hash routes are not separately crawlable. Solana code may only live under `src/pool/`, `src/app/`, `src/shared/` (ConnectorKit `SolanaProviders` + the `useClusterRpc`/`useHanseEnv` seam + the `sendInstruction` port live in `src/shared/`, mounted by the pool and app route modules only; the pool page binds tiers/deposits window to the on-chain mutual via `@riprap/hanse` — `src/pool/{mutual,useMutual}.ts`, per-cluster address map, no static fallback; `#/app` is reads-only v1 — wallet gate, member + claims via `src/app/{useMembership,useClaims}.ts`, reusing `src/pool/`'s mutual map and `useMutual` read; `#/app/file-claim` is the write surface — one `hanse::file_claim` via `@riprap/hanse`'s `buildFileClaim` + ADR-0031 evidence delivery) — the platform route's chunk stays `@solana/*`-free (lazy route modules keep chain code out of it). `pitch-seed-raise` has no test lane — the gate's `build`/`lint` legs are its only automated check.|
...
|Cross-repo pin|`programs/hanse/Cargo.toml` (accord `rev`, see Completion Gate), `@useaccord/sdk@0.1.0` (`apps/cli` + `tests` + `apps/landing` pool page — the juror-stake read off the mutual's subaccord; `@useaccord/sdk/evidence`'s `claimantEncrypt` in `apps/landing` `#/app/file-claim` route chunk only — ADR-0031 evidence delivery), `ACCORD_SO` artifact|The sibling accord checkout is an external dependency of both the program and the TS workspace.|
...
- **Landing routes** (a new hash route, or imports crossing `apps/landing/src/` subtrees): the landing is one Vite entry — `index.html` → the hash router in `src/main.tsx`; the platform route (`App.tsx`) must never import `@solana/*` — chain code lives under `src/pool/`, `src/app/`, `src/shared/` and is imported only by the lazy route modules (`src/{pool,app}/entry.tsx`), keeping it out of the platform chunk. A new route adds: a case in the `src/main.tsx` router (+ its `<title>`), a `src/<route>/entry.tsx`, a router-test case, and — if user-visible — its copy in `meta/marketing/03-website-copy/landing-page.md` plus a `public/llms.txt` entry (hash routes are not sitemap-addressable; if it replaces an old MPA path, leave a redirect stub under `public/<path>/index.html`), all in the same change.
