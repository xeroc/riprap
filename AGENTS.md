...
Riprap is a protocol for **mutuals on Solana** — permissionless protection pools: anyone can create a mutual on any terms (any entry amount, any policy content the group defines, finite or ongoing lifetime; recurring contributions on the roadmap); claims are adjudicated by the sister protocol Accord; the money stays the group's. Events are the pilot's shape, not the protocol's limit — first deployment: **Riprap: Blade Pool @ Breakpoint 2026**, 15–17 November 2026, Olympia Convention Centre, London (a knife-assault mutual for conference attendees; the only allowed prices $10/$20/$40 entry → $1,000/$2,000/$4,000 caps). This repo is a pnpm monorepo with a frontend half and an on-chain half: `packages/ui` (`@riprap/ui` — data-bound React SVG illustration kit with Storybook 10), `apps/landing` (`@riprap/landing` — static Vite + React site), and `programs/pool` (Anchor program `pool` — generic three-track mutual-pool primitive, localnet/LiteSVM only in v1) with its generated Codama client `packages/pool` (`@riprap/pool`). TypeScript strict, React 19, Vite, Vitest, Biome; Rust 1.89, Anchor 1.0.2. No web2 backend, no database, no environment variables.
